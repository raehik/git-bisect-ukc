...
--   * edit all commits' ancestors so they don't refer to filtered commits
--
...
git_repo_subgraph c_good c_bad g =
    ...
    in let sg' = git_repo_subgraph_remove_goods g sg c_good
    in     git_repo_subgraph_filter_ancestors sg'
...
git_repo_subgraph_filter_ancestors g = Map.map (\(c_parents, c_ancs) -> (c_parents, (Set.filter (\c -> Map.member c g) c_ancs))) g
            ...
            then
                ...
                in
                    ...
                    then
                        ...
                        if c_cur_rank >= floor ((fromIntegral (Map.size g)) / 2)
                        then (Just c_cur, g)
                        else git_repo_get_bisect_commit_calc_limit' g (c_cur, c_cur_rank) c_stack remaining_calcs
                    else git_repo_get_bisect_commit_calc_limit' g (c_best_current, c_best_current_rank) c_stack remaining_calcs
